...
class ImageUpload extends React.Component {
   ...
   */
  // have to rewrute this fucntion when doing for multiple image,we have to do deep check on each value
  componentWillReceiveProps(nextProps) {
    if (nextProps.defaultImages[0] !== this.props.defaultImages[0]) {
  ...
  onDropFile(e) {
    ...
    Promise.all(allFilePromises).then((newFilesData) => {
      ...
      this.setState({ pictures: dataURLs, files: files }, () => {
        if (files.length === 0)
          this.props.onChange([false], this.state.pictures);
      });
    });
  }
  ...
  removeImage(picture) {
    ...
    this.setState({ pictures: filteredPictures, files: filteredFiles }, () => {
      if (this.state.files.length === 0)
        this.props.onChange([false], this.state.pictures);
      else this.props.onChange(this.state.files, this.state.pictures);
    });
  }
